...
fn bench_part(c: &mut Criterion, day: u32, part: u32, input: &str) {
    c.bench_function(&format!("day{day:02}/part{part}"), |b| {
        b.iter(|| {
            let answer = Year2016::solve(day, part, black_box(input)).unwrap_or_else(|error| {
                panic!("Year 2016 day {day:02} part {part} failed: {error}")
            });
            black_box(answer);
        });
    });
